// auth.js — MongoDB version (talks to Express backend)

const API = 'http://localhost:3001/api'; // ✅ Change to your deployed backend URL in production
...
async function handleLogin(e) {
  ...
  try {
    ...
    if (!res.ok) {
      showAlert(alertEl, 'error', '✗ ' + data.error);
      return;
    }

    setAuthState({ name: data.name, email: data.email });
    closeLogin();
    updateAuthUI();
